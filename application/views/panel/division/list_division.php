<div class="card mb-4">
    <div class="card-header"> Division Management</a></div>
...
<?php $this->load->view('include/modal', ["size" => "md", "title" => "Division", "vertical" => true, "add_btn" => '<button type="button" class="btn btn-primary" onclick="save(this);">Save changes</button>']); ?>
<script>
    let tdatable;

    function edit(btn) {
        $("#gm_title").html("Edit Division");
    ...
    function create() {
        $("#gm_title").html("Create Division");
        $.get("<?= base_url("division/create") ?>", function(data) {
            $('#generalModal').modal('toggle')
            $("#gm_body").html(data);
            appLoadSelect2('#generalModal');
        });
    ...
    function save(btn) {
        ...
        if (getval("division_id") == "") {
            $.post("<?= base_url("division/data/add") ?>", {
                "division_name": getval("division_name"),
                "division_company": getval("division_company"),
            }, function(data) {
            ...
            $.post("<?= base_url("division/data/edit") ?>", {
                "division_id": getval("division_id"),
                "division_name": getval("division_name"),
                "division_company": getval("division_company"),
            }, function(data) {
                $('#generalModal').modal('hide')
    ...
    function remove(id) {
        Swal.fire({
            ...
            if (result.isConfirmed) {
                $.post("<?= base_url("division/data/delete") ?>", {
                    "division_id": id,
                }, function(data) {
                    Swal.fire(
                        'Removed!',
    ...
    $(document).ready(() => {
        tdatable = $('#companylist').DataTable({
            ...
            ajax: {
                url: '<?= base_url("division/data/list") ?>',
                dataSrc: 'data',
            },
            ...
            columns: [{
                ...
                },
                {
                    ...
                    render: function(data, type, row) {
                        return `
                            <button <?= ($permission->u ? "" : "disabled") ?> class="btn text-white btn-sm btn-success me-2" onclick="edit(this)" href="<?= base_url("division/edit/") ?>${data}" data-coreui-toggle="tooltip" data-coreui-placement="bottom" title="Edit"><i class="fas fa-pencil-alt"></i></button>
